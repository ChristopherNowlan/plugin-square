...
```ts
import squarePlugin from 'sample-plugin';
...
The `squarePlugin` has already been installed to the `payload.config()` file in this project.
...
If your plugin uses packages or dependencies that are not browser compatible (fs, square, nodemailer, etc), you will need to alias them using your bundler to prevent getting errors in build.
